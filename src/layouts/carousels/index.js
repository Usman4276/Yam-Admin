...
function Carousels({ title }) {
  ...
  const fetchAllSalesAndCarousels = async (selectedSale) => {
    ...
    const dbCarouselData = getAllCarousel.docs.map((items) => ({
      id: items.id,
      ...items.data(),
    }));
    let carouselData = {};
    ...
    try {
      ...
      if (carouselId) {
        try {
          ...
          console.log(
            "🚀 ~ file: index.js:194 ~ onAddCarousel ~ error:",
            error
          );
        }
      }
      if (!carouselId && salesDbBrandData && salesDbData) {
        try {
          const result = await addDoc(collection(db, "carousels"), {
            carousels: [
              {
                sale: {
                  brand: {
          ...
          console.log(
            "🚀 ~ file: index.js:230 ~ onAddCarousel ~ error:",
            error
          );
        }
      }
